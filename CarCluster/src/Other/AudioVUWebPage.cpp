...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{font-family:Arial,sans-serif;margin:0;background:var(--bg);color:var(--text)}
    ...
    .readout{padding:14px;background:#f8fafc;border:1px solid var(--line);border-radius:8px}.readout.master{grid-column:1/-1}.readout-head{display:flex;justify-content:space-between;gap:10px;margin-bottom:10px;font-weight:700}.value{font-variant-numeric:tabular-nums;color:var(--blue)}
...
<main>
  ...
  <section>
    ...
    <div class="grid">
      ...
      <div class="field"><label for="attack">Attack: <span id="attackValue">5 ms</span></label><input id="attack" type="range" min="0" max="120" step="1" value="5"></div>
      ...
      <div class="field"><label for="peakHold">Needle peak hold: <span id="peakHoldValue">90 ms</span></label><input id="peakHold" type="range" min="0" max="250" step="5" value="90"></div>
  ...
  <section>
    <h2>Live response</h2>
      ...
      <div class="readout master"><div class="readout-head"><span>MASTER - consumption scale</span><span id="masterValue" class="value">0% / 0.0 l/100 km</span></div><div class="meter"><div id="masterFill" class="fill"></div></div></div>
    </div>
    <p class="muted">Bass transients are sampled at about 100 Hz and needle targets are sent up to about 50 times per second. Peak hold keeps short kicks high long enough for the physical motors to react; it does not change the motors' internal speed. Set Attack and Release to 0 ms for immediate software response. Master volume is mapped linearly: 0% = 0, 50% = 10, and 100% = 20 l/100 km. The cluster derives this economy display from CAN distance/consumption data, so its own filtering still applies.</p>
  </section>
...
(() => {
  ...
  const ui={device:byId('device'),mode:byId('mode'),sensitivity:byId('sensitivity'),attack:byId('attack'),release:byId('release'),peakHold:byId('peakHold'),linked:byId('linked'),swap:byId('swap'),ignition:byId('ignition'),devicesButton:byId('devicesButton'),startButton:byId('startButton'),stopButton:byId('stopButton'),status:byId('status'),leftFill:byId('leftFill'),rightFill:byId('rightFill'),masterFill:byId('masterFill'),speedValue:byId('speedValue'),rpmValue:byId('rpmValue'),masterValue:byId('masterValue'),secureWarning:byId('secureWarning')};
  let audioContext=null,stream=null,meterNode=null,silentGain=null,fallbackTimer=0,running=false,originalState=null,maxSpeed=260,maxRPM=6000,analysers=[],timeBuffers=[],frequencyBuffers=[],envelopes=[0,0],heldPeaks=[0,0],peakHoldUntil=[0,0],masterEnvelope=0,lastFallbackTime=0,lastSendTime=0,pendingState=null,writePromise=null;
...
class CarClusterMeterProcessor extends AudioWorkletProcessor {
  constructor(){
    ...
    this.highPass=[0,0];
    this.lowPass=[0,0];
    this.reportSamples=Math.max(128,Math.round(sampleRate/100));
  ...
  process(inputs,outputs){
    ...
    for(let i=0;i<frames;i+=1){
      ...
      for(let channel=0;channel<2;channel+=1){
        ...
        this.bassPeak[channel]=Math.max(this.bassPeak[channel],Math.abs(low));
      }
    }
    this.samples+=frames;
    if(this.samples>=this.reportSamples){
      this.port.postMessage({
        ...
        bass:[
          Math.max(Math.sqrt(this.bassPower[0]/this.samples),this.bassPeak[0]*.7),
          Math.max(Math.sqrt(this.bassPower[1]/this.samples),this.bassPeak[1]*.7)
        ],
...
`;
  ...
  function smoothValue(current,target,elapsedMs){const timeMs=target>current?Number(ui.attack.value):Number(ui.release.value);const next=timeMs<=0?target:current+(target-current)*(1-Math.exp(-elapsedMs/timeMs));return next<.004?0:next}
  function holdNeedlePeak(index,target,now){const holdMs=Number(ui.peakHold.value);if(holdMs<=0){heldPeaks[index]=target;peakHoldUntil[index]=now;return target}if(target>=heldPeaks[index]){heldPeaks[index]=target;peakHoldUntil[index]=now+holdMs;return target}if(now<peakHoldUntil[index])return heldPeaks[index];heldPeaks[index]=target;return target}
  function updateReadouts(left,right,speed,rpm,master=0,consumption=0){ui.leftFill.style.width=(Math.max(0,Math.min(1,left))*100).toFixed(1)+'%';ui.rightFill.style.width=(Math.max(0,Math.min(1,right))*100).toFixed(1)+'%';ui.masterFill.style.width=(Math.max(0,Math.min(1,master))*100).toFixed(1)+'%';ui.speedValue.textContent=speed+' / '+maxSpeed+' km/h';ui.rpmValue.textContent=rpm+' / '+maxRPM+' RPM';ui.masterValue.textContent=Math.round(master*100)+'% / '+consumption.toFixed(1)+' l/100 km'}
  function processLevels(full,bass,elapsedMs){if(!running)return;const bassMode=ui.mode.value==='bass',source=bassMode?bass:full,now=performance.now(),elapsed=Math.min(100,Math.max(1,Number(elapsedMs)||10));let leftTarget=targetLevel(source[0],bassMode),rightTarget=targetLevel(source[1],bassMode);if(ui.linked.checked)leftTarget=rightTarget=Math.max(leftTarget,rightTarget);if(ui.swap.checked)[leftTarget,rightTarget]=[rightTarget,leftTarget];leftTarget=holdNeedlePeak(0,leftTarget,now);rightTarget=holdNeedlePeak(1,rightTarget,now);envelopes[0]=smoothValue(envelopes[0],leftTarget,elapsed);envelopes[1]=smoothValue(envelopes[1],rightTarget,elapsed);const fullLeft=Math.max(0,Number(full[0])||0),fullRight=Math.max(0,Number(full[1])||0),masterTarget=targetLevel(Math.sqrt((fullLeft*fullLeft+fullRight*fullRight)/2),false);masterEnvelope=smoothValue(masterEnvelope,masterTarget,elapsed);const left=envelopes[0],right=envelopes[1],speed=Math.round(left*maxSpeed),rpm=Math.round(right*maxRPM),consumption=masterEnvelope*20;updateReadouts(left,right,speed,rpm,masterEnvelope,consumption);if(now-lastSendTime>=SEND_INTERVAL_MS){const nextState={speed,rpm,fuel_consumption_l_per_100km:consumption,fuel_consumption_active:true};if(ui.ignition.checked)nextState.ignition=true;scheduleState(nextState);lastSendTime=now}}
  ...
  async function start(){if(running)return;if(!window.isSecureContext||!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia){ui.secureWarning.classList.remove('hidden');throw new Error('The browser blocked audio capture. Mark this ESP32 origin as secure first.')}if(!ui.device.options.length||!ui.device.value)await discoverDevices(true);originalState=await fetchState();stream=await navigator.mediaDevices.getUserMedia(audioConstraints(ui.device.value));const AudioContextClass=window.AudioContext||window.webkitAudioContext;audioContext=new AudioContextClass({latencyHint:'interactive'});await audioContext.resume();envelopes=[0,0];heldPeaks=[0,0];peakHoldUntil=[0,0];masterEnvelope=0;lastFallbackTime=0;lastSendTime=0;running=true;ui.startButton.disabled=true;ui.stopButton.disabled=false;ui.devicesButton.disabled=true;ui.device.disabled=true;try{localStorage.setItem('carcluster-vu-device',ui.device.value)}catch(_){}const backgroundClock=await startAudioClock(audioContext.createMediaStreamSource(stream));setStatus(backgroundClock?'VU is running. Short peaks are held for the needles; you may switch to Spotify or minimize Chrome.':'VU is running with the compatibility timer. Keep this tab visible for the smoothest response.','ok')}
  async function stop(restoreState){const wasRunning=running;running=false;if(fallbackTimer)clearInterval(fallbackTimer);fallbackTimer=0;pendingState=null;if(meterNode){meterNode.port.onmessage=null;try{meterNode.disconnect()}catch(_){}}meterNode=null;if(silentGain){try{silentGain.disconnect()}catch(_){}}silentGain=null;if(stream)stream.getTracks().forEach(track=>track.stop());stream=null;if(audioContext){try{await audioContext.close()}catch(_){}}audioContext=null;analysers=[];timeBuffers=[];frequencyBuffers=[];try{if(writePromise)await writePromise;if(restoreState&&originalState)await postStateNow({speed:Number(originalState.speed)||0,rpm:Number(originalState.rpm)||0,ignition:Boolean(originalState.ignition),fuel_consumption_l_per_100km:Number(originalState.fuel_consumption_l_per_100km)||0,fuel_consumption_active:Boolean(originalState.fuel_consumption_active)});if(wasRunning)setStatus('VU stopped and the original cluster state was restored.','ok')}finally{originalState=null;envelopes=[0,0];heldPeaks=[0,0];peakHoldUntil=[0,0];masterEnvelope=0;updateReadouts(0,0,0,0,0,0);ui.startButton.disabled=false;ui.stopButton.disabled=true;ui.devicesButton.disabled=false;ui.device.disabled=false}}
  ...
  [['sensitivity','sensitivityValue',value=>Number(value).toFixed(2)+'x'],['attack','attackValue',value=>value+' ms'],['release','releaseValue',value=>value+' ms'],['peakHold','peakHoldValue',value=>value+' ms']].forEach(([inputId,outputId,format])=>{const input=byId(inputId),output=byId(outputId);input.addEventListener('input',()=>{output.textContent=format(input.value)})});
  window.addEventListener('pagehide',()=>{if(!running||!originalState||!navigator.sendBeacon)return;running=false;const restore=JSON.stringify({speed:Number(originalState.speed)||0,rpm:Number(originalState.rpm)||0,ignition:Boolean(originalState.ignition),fuel_consumption_l_per_100km:Number(originalState.fuel_consumption_l_per_100km)||0,fuel_consumption_active:Boolean(originalState.fuel_consumption_active)});navigator.sendBeacon('/api/state',new Blob([restore],{type:'application/json'}))});
