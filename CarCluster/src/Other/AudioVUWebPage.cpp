...
<main>
  ...
  <section>
    ...
    <div class="grid">
      ...
      <div class="field"><label for="release">Release: <span id="releaseValue">160 ms</span></label><input id="release" type="range" min="40" max="600" step="5" value="160"></div>
  ...
  <section>
    ...
    <p class="muted">Updates are limited to about 25 Hz and requests are coalesced. Stop restores the speed, RPM, and ignition state that were active before the test.</p>
  </section>
...
(() => {
  ...
  let audioContext=null,stream=null,frameId=0,running=false,originalState=null,maxSpeed=260,maxRPM=6000,analysers=[],timeBuffers=[],frequencyBuffers=[],envelopes=[0,0],lastFrameTime=0,lastSendTime=0,pendingState=null,writePromise=null;
  ...
  function rmsDb(analyser,buffer){analyser.getFloatTimeDomainData(buffer);let sum=0;for(let i=0;i<buffer.length;i+=1)sum+=buffer[i]*buffer[i];return 20*Math.log10(Math.max(Math.sqrt(sum/buffer.length),.000001))}
  function bassDb(analyser,buffer){analyser.getFloatFrequencyData(buffer);const binHz=audioContext.sampleRate/analyser.fftSize,first=Math.max(1,Math.floor(35/binHz)),last=Math.min(buffer.length-1,Math.ceil(180/binHz));let power=0,count=0;for(let i=first;i<=last;i+=1){power+=Math.pow(10,buffer[i]/10);count+=1}return 10*Math.log10(Math.max(power/Math.max(1,count),.0000000001))}
  function targetLevel(channel){const bassMode=ui.mode.value==='bass',db=bassMode?bassDb(analysers[channel],frequencyBuffers[channel]):rmsDb(analysers[channel],timeBuffers[channel]),adjustedDb=db+20*Math.log10(Math.max(.01,Number(ui.sensitivity.value))),floor=bassMode?-68:-52,ceiling=bassMode?-16:-5;return Math.max(0,Math.min(1,(adjustedDb-floor)/(ceiling-floor)))}
  function smoothEnvelope(index,target,elapsedMs){const timeMs=target>envelopes[index]?Number(ui.attack.value):Number(ui.release.value);envelopes[index]+=(target-envelopes[index])*(1-Math.exp(-elapsedMs/Math.max(1,timeMs)));if(envelopes[index]<.004)envelopes[index]=0;return envelopes[index]}
  ...
  function render(now){if(!running)return;const elapsed=Math.min(100,Math.max(1,now-(lastFrameTime||now-16)));lastFrameTime=now;let leftTarget=targetLevel(0),rightTarget=targetLevel(1);if(ui.linked.checked)leftTarget=rightTarget=Math.max(leftTarget,rightTarget);if(ui.swap.checked)[leftTarget,rightTarget]=[rightTarget,leftTarget];const left=smoothEnvelope(0,leftTarget,elapsed),right=smoothEnvelope(1,rightTarget,elapsed),speed=Math.round(left*maxSpeed),rpm=Math.round(right*maxRPM);updateReadouts(left,right,speed,rpm);if(now-lastSendTime>=SEND_INTERVAL_MS){const nextState={speed,rpm};if(ui.ignition.checked)nextState.ignition=true;scheduleState(nextState);lastSendTime=now}frameId=requestAnimationFrame(render)}
  async function start(){if(running)return;if(!window.isSecureContext||!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia){ui.secureWarning.classList.remove('hidden');throw new Error('The browser blocked audio capture. Mark this ESP32 origin as secure first.')}if(!ui.device.options.length||!ui.device.value)await discoverDevices(true);originalState=await fetchState();stream=await navigator.mediaDevices.getUserMedia(audioConstraints(ui.device.value));const AudioContextClass=window.AudioContext||window.webkitAudioContext;audioContext=new AudioContextClass({latencyHint:'interactive'});await audioContext.resume();const source=audioContext.createMediaStreamSource(stream),splitter=audioContext.createChannelSplitter(2);source.connect(splitter);analysers=[audioContext.createAnalyser(),audioContext.createAnalyser()];analysers.forEach(analyser=>{analyser.fftSize=1024;analyser.smoothingTimeConstant=0;analyser.minDecibels=-100;analyser.maxDecibels=-6});splitter.connect(analysers[0],0);splitter.connect(analysers[1],1);timeBuffers=analysers.map(analyser=>new Float32Array(analyser.fftSize));frequencyBuffers=analysers.map(analyser=>new Float32Array(analyser.frequencyBinCount));envelopes=[0,0];lastFrameTime=0;lastSendTime=0;running=true;ui.startButton.disabled=true;ui.stopButton.disabled=false;ui.devicesButton.disabled=true;ui.device.disabled=true;try{localStorage.setItem('carcluster-vu-device',ui.device.value)}catch(_){}setStatus('VU is running. Audio stays inside this browser.','ok');frameId=requestAnimationFrame(render)}
  async function stop(restoreState){const wasRunning=running;running=false;if(frameId)cancelAnimationFrame(frameId);frameId=0;pendingState=null;if(stream)stream.getTracks().forEach(track=>track.stop());stream=null;if(audioContext){try{await audioContext.close()}catch(_){}}audioContext=null;analysers=[];try{if(writePromise)await writePromise;if(restoreState&&originalState)await postStateNow({speed:Number(originalState.speed)||0,rpm:Number(originalState.rpm)||0,ignition:Boolean(originalState.ignition)});if(wasRunning)setStatus('VU stopped and the original cluster state was restored.','ok')}finally{originalState=null;envelopes=[0,0];updateReadouts(0,0,0,0);ui.startButton.disabled=false;ui.stopButton.disabled=true;ui.devicesButton.disabled=false;ui.device.disabled=false}}
