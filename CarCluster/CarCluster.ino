...
void webDashboardBMWFTestControls(HttpConnection *connection, int event, void *eventData) {
  ...
  httpReply(
    ...
    "<p class='meta'><strong>Clock setting is not working on the verified 6WA cluster.</strong> The remaining CAN/UDS clock functions are research code. Normal runtime date/time broadcasts are disabled because none of the tested payloads set the clock.</p>"
    "<label><span>0x39E 6WA candidate - retained research control</span><input type='checkbox' name='time39e_6wa'%s disabled></label>"
    "<p class='meta'>The candidate payload remains documented in the source, but it is not sent by the normal update loop.</p>"
    "<label><span>Clear CC-ID 167 Reset Clock - default ON</span><input type='checkbox' name='clearcc167'%s></label>"
    "<p class='meta'>Repeatedly sends the 0x5C0 clear state for CC-ID 167. This suppresses the yellow Reset Clock notification; it does not set or reset the displayed time.</p>"
    "</section>"
